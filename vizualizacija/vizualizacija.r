...
graf_SLO_Kolicina_proizvodov <- podatki_Slovenija %>% group_by(pridelek) %>%  
  ...
  ggtitle("Količina ton proizvedbe na hektarski meter vsakega proizvoda, v celi Sloveniji: 2010-2019") + xlab("Vrsta pridelka") + ylab("Količina t/ha proizvoda")

graf_SLO_Kolicina_proizvodov_na_regio <- podatki_Regija %>% group_by(pridelek) %>%  
  ggplot(aes(x=pridelek, y=Kolicina)) + geom_boxplot() + theme(axis.text.x = element_text(
    color="#000000", size=8, angle=90)) +
  ggtitle("Količina t/ha proizvedbe vsakega proizvoda, za vsako regijo: 2010-2019") + xlab("pridelka") + ylab("Količina t/ha proizvoda")
...
zemljevid <- ggplot() +
  ...
  xlab("asd") + ylab("asdasd") + ggtitle('Količina proizvoda v posameznih regijah v tonah') + 
  theme(axis.title=element_blank(), axis.text=element_blank(), axis.ticks=element_blank(), panel.background = element_blank()) 
